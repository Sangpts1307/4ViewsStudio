                        ...
                        @foreach ($appointments as $index => $appointment)
                            <tr>
                                ...
                                <th>{{ \Carbon\Carbon::parse($appointment->shift->start_time)->format('H:i') }} -
                                    {{ \Carbon\Carbon::parse($appointment->shift->end_time)->format('H:i') }}</th>
                                ...
                                <td>
                                    @if ($appointment->availableStaffs && $appointment->availableStaffs->isNotEmpty())
                                        <form action="{{ url('/admin/appointment-sche') }}" method="POST"
                                            ...
                                            <select name="staff_id" id="staff_id_{{ $appointment->id }}" required
                                                onchange="this.form.submit()">
                                                @if ($appointment->availableStaffs->count() == 1)
                                                    <option value="" {{ !$appointment->staff_id ? 'selected' : '' }}>
                                                        Chọn
                                                        thợ chụp</option>
                                ...
                                <td><button class="save-btn">🖨️</button></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
